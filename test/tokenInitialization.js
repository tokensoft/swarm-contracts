...
contract('Swarm Token Init', async (accounts) => {
  it('initial properties should be set', async () => {
    ...
    assert.equal(await token.VESTING_PERIOD_TIME(), 42 * 24 * 60 * 60, '42 days in seconds')
    assert.equal(await token.VESTING_TOTAL_PERIODS(), 8, 'Vesting periods should by 8')
